...
public class ItemSheetVersionsController : ControllerBase
{
    ...
    public async Task<ActionResult<ItemSheetVersion>> GetItemSheetVersion(int id)
    {
        ...
        if (UsersLogic.IsUserAuthed(authId, accessToken, "Approver", _context))
        {
            ...
            {
                var creUser = await _context.Users.Where(u => u.Guid == outputSheet.CreatedbyuserGuid)
                    .FirstOrDefaultAsync();
            ...
            {
                var creUser = await _context.Users.Where(u => u.Guid == outputSheet.FirstapprovalbyuserGuid)
                    .FirstOrDefaultAsync();
                outputSheet.createdby = creUser.Firstname + " " + creUser.Lastname;
            }
            ...
            {
                var creUser = await _context.Users.Where(u => u.Guid == outputSheet.SecondapprovalbyuserGuid)
                    .FirstOrDefaultAsync();
                outputSheet.createdby = creUser.Firstname + " " + creUser.Lastname;
            }
    ...
    public async Task<IActionResult> PutItemSheetVersion(int id, ItemSheetVersion itemSheetVersion)
    {
        ...
        {
            if (id != itemSheetVersion.Id) return BadRequest();
