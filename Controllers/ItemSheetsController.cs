...
public class ItemSheetsController : ControllerBase
{
    ...
    public async Task<ActionResult<IteSheet>> PutItemSheet(Guid guid, [FromBody] IteSheetInput item)
    {
        ...
        if (UsersLogic.IsUserAuthed(authId, accessToken, "Writer", _context))
        {
            ...
            var itemSheet = await _context.ItemSheet.Where(s => s.Guid == guid && s.Isactive == true)
                .FirstOrDefaultAsync();
            ...
            {
                var itemSheetApp = await _context.ItemSheetApproved.Where(s => s.Guid == guid && s.Isactive == true)
                    .FirstOrDefaultAsync();

                if (guid != item.Guid || itemSheetApp == null) return BadRequest();

                var newitemSheet = new ItemSheet();

                if (item.Guid != null) newitemSheet.Guid = item.Guid;

                if (item.Name != null) newitemSheet.Name = item.Name;

                if (item.Gmnotes != null) newitemSheet.Gmnotes = item.Gmnotes;
                ...
                if (item.Reason4edit != null) newitemSheet.Reason4edit = item.Reason4edit;
                ...
                var outputItem = Item.CreateItem(newitemSheet);
                return outputItem;
            }
            ...
            if (item.Img1 != null) itemSheet.Img1 = item.Img1;
