...
public class ItemSheetsController : ControllerBase
{
    ...
    public ItemSheetsController(NexusLarpLocalContext context, IMinioClient minio, IOptions<MinioOptions> minioOptions)
    {
        _context = context;
        _minio = minio;
...
*/
    ...
    [HttpGet("MultiPick/")]
    [Authorize]
    public async Task<ActionResult<List<IteSheet>>> GetItemSheetList([FromQuery] ItemSheetInput input)
    {
        ...
        if (UsersLogic.IsUserAuthed(HttpContext.User, "Reader", _context))
        {
            ...
            if (input.A != null)
            {
                ...
                {
                    var outputAppedItem =
                        await GetApprovedItemSheetOutput(guid, allowedTags, usersList, listItemTypes);
    ...
    public async Task<ActionResult<IEnumerable<List<IteSheet>>>> FullListWithTagsAndDeactive()
    {
        ...
        if (UsersLogic.IsUserAuthed(HttpContext.User, "Wizard", _context))
            try
            {
                ...
                var ActiveSheets = _context.ItemSheets.Where(x => x.Isactive == true).GroupBy(x => x.Guid)
                 .Select(x => new
                 {
                     guid = x.Key,
                     createdate = x.Max(row => row.Createdate)
                ...
                var activeSheetIDs = _context.ItemSheets.Join(ActiveSheets,
                   i => new { guid = i.Guid, createdate = i.Createdate },
                   s => new { s.guid, s.createdate }, (i, s) => i).Select(i => i.Id).ToList();
                ...
                var itemCounts = _context.ItemSheets.Select(ish => new {
                    ish.Guid,
                    ish.Id
                }).ToList();

                var allSheets = await _context.ItemSheets.Where(i => activeSheetIDs.Contains(i.Id) ||
                     (allSheetIDs.Contains(i.Id) && !activeSheetIDs.Contains(i.Id)))
                    .Select(x => new ItemSheetDO
                    {
                        Sheet = new ItemSheet
                        {
                ...
                {
                    item.CountVersions = itemCounts.Where(ic => ic.Guid == item.Guid).ToList().Count;
                    var res = isheetapprov.Where(isa => isa.Guid == item.Guid).FirstOrDefault();
                    if (res != null)
                    {
                        item.Isactive = true;
                    }
    ...
    public async Task<ActionResult<IEnumerable<List<IteSheet>>>> FullListWithTagsAndDeactive(Guid guid)
    {
        ...
        if (UsersLogic.IsUserAuthed(HttpContext.User, "Wizard", _context))
            try
            {
                ...
                foreach (var item in isheetapprov)
                {
                    var res = outPutList.Where(opt => opt.Id == item.ItemsheetId).FirstOrDefault();
                    if (res != null)
                    {
                        res.Isactive = true;
                    }
    ...
    [Authorize]
    public async Task<ActionResult<IteSheet>> GetItemSearchPartial(string input)
    {
        var authId = HttpContext.User.FindFirstValue("sub") ?? HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier);
        ...
        if (UsersLogic.IsUserAuthed(HttpContext.User, "Reader", _context))
        {
            ...
            searchLogic.sheets = await _context.ItemSheets
            ...
            && searchLogic.orAttrisheets.Contains(c.Guid))
                    .Select(x => new ItemSheetDO
                    {
                        ...
                        TagList = x.ItemSheetTags.Select(cst => cst.Tag)
    ...
    public async Task<IActionResult> ApproveItemSheet(Guid guid)
    {
        ...
        if (UsersLogic.IsUserAuthed(HttpContext.User, "Approver", _context))
        {
            try
            {
                ...
                var result = _context.Users.Where(u => u.Email == authId).Select(u => u.Guid).FirstOrDefault();
        ...
        if (UsersLogic.IsUserAuthed(HttpContext.User, "Writer", _context))
            try
            {
                ...
                itemSheet.CreatedbyuserGuid =
                    _context.Users.Where(u => u.Email == authId).Select(u => u.Guid).FirstOrDefault();
              ...
              //  }
                ...
                var nwoutputItem = Item.CreateItem(itemSheet, usersList, listItemTypes);
    ...
    public async Task<ActionResult<ItemSheet>> DeacrtivateItemSheet(int id)
    {
        ...
        if (UsersLogic.IsUserAuthed(HttpContext.User, "Wizard", _context))
        {
            try
            {
                ...
                var itemSheetApps = await _context.ItemSheetApproveds
                     .Where(i => i.ItemsheetId == id).ToListAsync();
                foreach (var item in itemSheetApps)
                {
                    item.Isactive = false;
    ...
    [HttpPut("Activate/{id}")]
    [Authorize(Policy = "Wizard")]
    public async Task<ActionResult<ItemSheet>> AcrtivateItemSheet(int id)
    {
        var authId = HttpContext.User.FindFirstValue("sub") ?? HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier);
        if (UsersLogic.IsUserAuthed(HttpContext.User, "Wizard", _context))
        {
            try
            {
                var itemSheet = await _context.ItemSheets.Where(i => i.Id == id).FirstOrDefaultAsync();
                if (itemSheet == null) return NotFound();

                var relatedSheets = await _context.ItemSheets.Where(i => i.Guid == itemSheet.Guid &&
                i.Id != itemSheet.Id).ToListAsync();

                foreach(var sheet in relatedSheets)
                {
                    sheet.Isactive = false;
                    _context.ItemSheets.Update(sheet);
                }

                itemSheet.Isactive = true;
                _context.ItemSheets.Update(itemSheet);
                _context.SaveChanges();
    ...
    private async Task<IteSheet> GetApprovedItemSheetOutput(Guid guid, List<Guid?> allowedTags,
        List<User> usersList, List<ItemType> listItemTypes
        )
    {
        ...
        var outputItem = Item.CreateItem(itemSheet, usersList, listItemTypes);
